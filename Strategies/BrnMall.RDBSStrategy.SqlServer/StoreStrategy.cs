...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable AdminGetStoreList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[storeid],[temp1].[state],[temp1].[name],[temp1].[storerid],[temp1].[createtime],[temp2].[title],[temp3].[uid] FROM (SELECT TOP {0} [storeid],[state],[name],[storerid],[createtime] FROM [{1}stores] WHERE [storeid] NOT IN (SELECT TOP {2} [storeid] FROM [{1}stores] ORDER BY {3}) ORDER BY [storeid] DESC) AS [temp1] LEFT JOIN [{1}storeranks] AS [temp2] ON [temp1].[storerid]=[temp2].[storerid] LEFT JOIN [{1}users] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
